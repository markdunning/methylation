...
\item{\textbf{Ensure that sample groups are balanced among batches}}
\end{itemize}
\end{frame}
...
\includegraphics[width=12cm]{software.png}
...
\item{Visualisation and correction of batch effetcs}
\end{itemize}
\column{.5\textwidth}
\includegraphics[width=5cm]{champ.png}
\end{columns}
\end{frame}
