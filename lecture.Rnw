...
\item{Can also be run as part of the {\tt ChAMP} pipeline}
\end{itemize}
\end{frame}
...
dmp <- dmpFinder(M, pheno=pd$Sample_Group,type="categorical",shrinkVar=T)
head(dmp)
@
...
ggplot(df, aes(x = Sample_Group, y=value,fill=Sample_Group)) + geom_boxplot() + facet_wrap(~X1,scales="free")
@
